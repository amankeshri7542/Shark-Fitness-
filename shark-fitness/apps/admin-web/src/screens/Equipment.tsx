...
function CreateEquipmentDialog({ branches, activeBranchId, online, onClose }: { branches: Array<{ id: string; name: string }>; activeBranchId: string | null; online: boolean; onClose: () => void }) {
  ...
  return <Dialog title="Add equipment" onClose={onClose} actions={<DialogActions onClose={onClose} isPending={create.isPending} disabled={!online || !name.trim() || !assetTag.trim() || !branchId || !area.trim()} label="Add equipment" onConfirm={() => { setFieldError(null); if (!name.trim() || !assetTag.trim() || !area.trim()) { setFieldError('Name, asset identifier and area are required.'); return; } create.mutate(); }} />}><div className="flex flex-col gap-3"><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><Field label="Name" value={name} onChange={(event) => setName(event.target.value)} placeholder="Cable crossover" autoFocus /><Field label="Asset / QR identifier" value={assetTag} onChange={(event) => setAssetTag(event.target.value)} placeholder="KOR-021" /></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><SelectField label="Branch" value={branchId} onChange={setBranchId} options={branches.map((branch) => [branch.id, branch.name] as [string, string])} /><Field label="Area" value={area} onChange={(event) => setArea(event.target.value)} placeholder="Strength floor" /></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><Field label="Model" value={model} onChange={(event) => setModel(event.target.value)} /><Field label="Serial" value={serial} onChange={(event) => setSerial(event.target.value)} /><Field label="Vendor" value={vendor} onChange={(event) => setVendor(event.target.value)} /></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><Field label="Warranty until" type="date" value={warrantyUntil} onChange={(event) => setWarrantyUntil(event.target.value)} /><Field label="Service interval (days)" type="number" min={1} max={3650} value={serviceIntervalDays} onChange={(event) => setServiceIntervalDays(event.target.value)} /></div>{error ? <Panel tone="bad"><p className="px-3 py-2.5 text-[12px]">{error}</p></Panel> : null}</div></Dialog>;
}
...
function ReturnToServiceDialog({ equipment, online, onClose }: { equipment: EquipmentRow; online: boolean; onClose: () => void }) {
  ...
  return (
    <Dialog title="Return to service" onClose={onClose} actions={<DialogActions onClose={onClose} isPending={returnToService.isPending} disabled={!online || !note.trim()} label="Return to service" onConfirm={() => returnToService.mutate()} />}>
...
function Dialog({ title, actions, onClose, children }: { title: string; actions: ReactNode; onClose: () => void; children: ReactNode }) {
  return (
    <Modal open onClose={onClose} title={title} footer={actions} width="w-[min(640px,100%)]">
