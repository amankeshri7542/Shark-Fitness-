...
function ConfirmDialog({ kind, isPending, onClose, onConfirm }: { kind: 'day' | 'item' | 'archive'; isPending: boolean; onClose: () => void; onConfirm: () => void }) { const archive = kind === 'archive'; return <Dialog title={archive ? 'Archive published version?' : kind === 'day' ? 'Remove program day?' : 'Remove exercise from day?'} onClose={onClose} actions={<DialogActions onClose={onClose} isPending={isPending} label={archive ? 'Archive version' : 'Remove'} onConfirm={onConfirm} danger />}><p className="text-[13px] leading-relaxed text-foam-65">{archive ? 'Assigned members keep their frozen version. New assignments will no longer use this published version.' : kind === 'day' ? 'The day and all exercises inside it will be removed from this draft.' : 'This removes the prescription from the draft. Workout history is not changed.'}</p></Dialog>; }
